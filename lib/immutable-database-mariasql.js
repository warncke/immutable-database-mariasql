...
// if set this will be called when instantiating a new db connection to
// apply the automock wrapper to the connection instance
var automockFunction
...
function automock (setAutomockFunction) {
    ...
    if (setAutomockFunction !== undefined) {
        ...
        // set global value
        automockFunction = setAutomockFunction
        // return class
        return ImmutableDatabaseMariaSQL
    }
    // return current value
    return automockFunction
}
...
function logQueryError (dbQueryId, options, err) {
    ...
    this.logClient.log('dbResponse', {
        data: err && typeof err === 'object' && err.message ? err.message : err,
        dbQueryId: dbQueryId.id,
